...
def get_events(
    service, days_ahead: int = 7, time_min: datetime = None, time_max: datetime = None
) -> list:
    """
    fetch calendar events for the next x days
    returns a list of event dictionaries

    time_min: optional explicit lower bound (must be timezone-aware).
    Defaults to now if not given — pass an earlier bound to also pick up
    events that already ended (e.g. tasks truncated earlier today on
    completion), which Google's own timeMin filtering would otherwise
    silently exclude.

    time_max: optional explicit upper bound (must be timezone-aware).
    Defaults to now + days_ahead if not given — pass an explicit value
    when the caller has a real visible range (e.g. a calendar Week/Month
    view) rather than a simple day count.
    """
    now = datetime.now(timezone.utc)
    query_start = time_min if time_min else now
    query_end = time_max if time_max else now + timedelta(days=days_ahead)
...
def get_all_events(
    days_ahead: int = 7, time_min: datetime = None, time_max: datetime = None
) -> list:
    """
    ...
    personal_events = get_events(personal, days_ahead, time_min, time_max)
    for e in personal_events:
        e["calendar"] = "personal"

    school_events = get_events(school, days_ahead, time_min, time_max)
...
def get_free_slots(
    events: list, start_hour: int = 8, end_hour: int = 22, min_slot_minutes: int = 15
) -> list:
    """

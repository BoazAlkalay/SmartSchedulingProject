...
def update_task_with_event(task_title: str, event_id: str, scheduled_time: str):
    """
    Update the task frontmatter with the calendar event ID and scheduled time.
    """
...
def schedule_task(
    ...
    if preferred_start:
        ...
        try:
            start = datetime.strptime(
                f"{now.strftime('%Y-%m-%d')} {preferred_start}",
                "%Y-%m-%d %I:%M %p"
            )
    ...
    event_id = create_calendar_event(
        ...
        description=f"Scheduled by SmartScheduler"
    )
    
    # Update the task file
    update_task_with_event(task_title, event_id, slot['start'])
    ...
    return {
        ...
        "end": slot['end'],
        "event_id": event_id,
        "message": f"Scheduled '{task_title}' from {slot['start']} to {slot['end']}"
    }
